...
class DB {
 public:
  virtual uint64_t ResetMaxL0FilesStats(uint64_t&, uint64_t&, uint64_t&) {
    return 0;
  }
  ...
  virtual inline Status MyGet(const ReadOptions& options,
                              ColumnFamilyHandle* column_family, 
							  const Slice& key, 
                              std::string* value) {
    assert(value != nullptr);
    ...
    if (s.ok() && pinnable_val.IsPinned()) {
	  ...
	  send(sock_fds[0], &key_size, sizeof(size_t), 0);

	  send(sock_fds[1], key.data(), key.size(), 0);

	  size_t recv_size = 0;
	  while (recv_size < value_size) {
	    char buf[value_size-recv_size];
		ssize_t ret = recv(sock_fds[1], buf, sizeof(buf), 0);
        if (ret <= 0) {
		  s = Status::TimedOut();
		  return s;
		}

		value->append(buf, ret);
		recv_size += ret;

		if (value->substr(0, 8) == "NotFound") {
		  s = Status::NotFound();
		  return s;
		}
	  }
  ...
  virtual Status MyGet(const ReadOptions& options, 
                       const Slice& key, 
                       std::string* value) {
    return MyGet(options, DefaultColumnFamily(), key, value);
  }
...
#undef DeleteFile
  ...
  // added
  void InitSocketFDs(const std::vector<int>& sock_fds_) {
	sock_fds = sock_fds_;
  }
  void InitValueSize(const size_t& value_size_) {
