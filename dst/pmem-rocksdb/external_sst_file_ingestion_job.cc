...
Status ExternalSstFileIngestionJob::Prepare(
  ...
  for (IngestedFileInfo& f : files_to_ingest_) {
	// added
    uint32_t path_id_ = 0;
	if (!cfd_->ioptions()->cf_paths.empty()) {
	  int sp = f.external_file_path.rfind("/");
	  const std::string external_file_dir = f.external_file_path.substr(0, sp);

	  for (size_t i = 0; i < cfd_->ioptions()->cf_paths.size(); i++) {
	    const std::string cf_path_tmp = cfd_->ioptions()->cf_paths[i].path;
		if (cf_path_tmp == external_file_dir) {
		  break;
		 }
	...
	f.fd = FileDescriptor(next_file_number++, path_id_, f.file_size);
    f.copy_file = false;
...
Status ExternalSstFileIngestionJob::Run() {
  ...
  for (IngestedFileInfo& f : files_to_ingest_) {
	...
	f.picked_level = ingestion_options_.external_level; // added
