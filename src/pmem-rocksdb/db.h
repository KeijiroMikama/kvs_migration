...
class DB {
 public:
  ...
  virtual inline Status MyGet(const ReadOptions& options,
	...
	Status s;
	size_t key_size = 0;
	ssize_t ret = recv(sock_fds[0], &key_size, sizeof(size_t), 0);
	if (ret <= 0) {
	  s = Status::TimedOut();
	  return s;
	}
	...
	size_t recv_size = 0;
	std::string key;
	while (recv_size < key_size) {
	  char buf[key_size-recv_size];
	  ret = recv(sock_fds[1], buf, sizeof(buf), 0);
	  if (ret <= 0) {
	    s = Status::TimedOut();
		return s;
	  }
    ...
    send(sock_fds[1], value->c_str(), value->size(), 0);
  ...
  virtual Status MyGet(const ReadOptions& options, 
                       std::string* value) {
    return MyGet(options, DefaultColumnFamily(), value);
  }
